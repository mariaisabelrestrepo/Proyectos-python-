...
#########################################################################
cantidad_temp_max_prom=[]
for i in range(len(output)):
    uno=[output[i][0],str(output[i][2])]
    cantidad_temp_max_prom.append(uno)
    
idx, max_value= max(cantidad_temp_max_prom, key=lambda item: item[1])

segunda_lista=[]

for i in range(len(cantidad_temp_max_prom)):
    if cantidad_temp_max_prom[i][1] == max_value:
        
        fe=cantidad_temp_max_prom[i][0]
        segunda_lista.append(fe)
...
for i in range(len(output)):
    uno=[output[i][0],str(output[i][3])]
    precipitacion_minima.append(uno)

idx, min_value= min(precipitacion_minima, key=lambda item: item[1])

tercera_lista=[]

for i in range(len(precipitacion_minima)):
    if precipitacion_minima[i][1] == min_value:
        
        fe=precipitacion_minima[i][0]
        tercera_lista.append(fe)
...
for i in range(len(output)):
    uno=[output[i][0],str(output[i][3])]
    precipitacion_maxima.append(uno)

idx, max_value= max(precipitacion_minima, key=lambda item: item[1])
cuarta_lista=[]
for i in range(len(precipitacion_maxima)):
    if precipitacion_maxima[i][1] == max_value:
        
        fe=precipitacion_maxima[i][0]
        cuarta_lista.append(fe)
...
#########################################################################
cantidad_dias_soleados=[]
for i in range(len(output)):
    uno=[output[i][0],str(output[i][6])]
    cantidad_dias_soleados.append(uno)

idx, max_value= max(cantidad_dias_soleados, key=lambda item: item[1])

quinta_lista=[]

for i in range(len(cantidad_dias_soleados)):
    if cantidad_dias_soleados[i][1] == max_value:
        fe=cantidad_dias_soleados[i][0]
        quinta_lista.append(fe)
